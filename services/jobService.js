  ...
  agenda.define('generate-html', async (job) => {
    ...
    try {
      ...
      for (let i = 0; i < totalPages; i++) {
        ...
        // Render page to PNG (saved permanently)
        ...
        // Extract chapter/section from vision HTML
        const h2Match = html.match(/<h2[^>]*>([^<]+)<\/h2>/);
        const h3Match = html.match(/<h3[^>]*>([^<]+)<\/h3>/);

        page.htmlContent = html;
        if (h2Match) page.chapterTitle = h2Match[1];
        if (h3Match) page.sectionTitle = h3Match[1];
        page.hasEquations = true;
        page.hasImages = true; // page PNG always available
        await page.save();

        const progress = 50 + Math.round(((i + 1) / totalPages) * 50);
        book.processingProgress = progress;
        await book.save();
        jobDoc.progress = progress;
        await jobDoc.save();
