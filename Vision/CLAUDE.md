...
**CRITICAL — Jony can only reliably read ~0.5 screen of terminal output**
- Jony's terminal scrollback is effectively broken: past the last ~0.5 screen, earlier output is unrecoverable (no scroll-up history). Anything you put only in chat past that line is INVISIBLE to him.
- Consequence: any report, spec, numerical table, logic breakdown, business analysis, cost rollup, or anything Jony needs to SIGN OFF on must be written to a file BEFORE you ask for sign-off. The chat message should contain (a) a one-line pointer to the file and (b) at most 10 lines of bullet summary.
- File location rules:
  - Numerical/benchmark/audit output → `reports/YYYY-MM-DD/<slug>.md`
  - Architectural specs / judge loops / pipeline designs → `reports/Notes rewrite and vision/<slug>-spec.md` or a topical subfolder under `reports/`
  - Business / strategy / monetization → `Vision/<topic>.md`
  - One-line status updates that don't need sign-off → chat is fine
- Default to over-saving. A small file is always recoverable; a big chat message scrolled past is not.
- When you link to a file, give the ABSOLUTE path from the repo root, e.g. `reports/2026-04-13/figure-judge-loop-spec.md`, so Jony can open it in Finder or Cursor without searching.

**Debug screenshots convention**
- Debug screenshots live in `reports/debug-pix/`. The `.gitignore` has an explicit exception for `reports/debug-pix/*.{png,jpg,jpeg}` so they're committed to git and visible to other LLMs working in the repo across sessions.
- When Jony hands you new screenshots, copy them into `reports/debug-pix/` and commit with a brief "debug-pix: YYYY-MM-DD <slug>" message so future sessions can see the visual state.
- The `reports/debug-pix/older pics/` subfolder is ignored (archive). If the repo size starts pushing the GitHub free tier (typically at ~1 GB), flag it to Jony BEFORE billing kicks in so he can decide whether to download and purge older screenshots rather than be surprised by a charge.
