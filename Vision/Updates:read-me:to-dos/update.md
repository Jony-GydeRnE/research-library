...
Note: when file reaches 1,500 lines, make a 2nd update.md file.
...
## 2026-04-13 — Notes-rewrite pipeline v1 + permanent auto-recovery for dropped vision pages

**Session outcome:** Pivoted the notes pipeline from "vision-transcribe the page verbatim" to "rewrite the page as clean expository HTML/LaTeX in Rodina's voice, grounded in the linked source paper." Shipped the first working version end-to-end, ran it against the Lagrangians notes book, and preview-rendered it. Also fixed a silent data-loss bug (33 pages dropped from the notes book due to TPM 429s never retried) both as a one-shot repair and as a durable auto-recovery hook that will catch the same failure mode for any future upload. Commit `9f7009e` on `main`. Full rewrite pass on the 65-page notes book is in-flight; partial results captured below.

### Why the pivot — the old approach was producing garbage

Screenshots earlier in the session showed the downstream rendering of notes pages: "undefined" pages, `Missing \left or \right` MathJax errors, figures as nonsense black boxes, handwritten diagrams not surviving transcription at all. The root cause was that vision-transcribing sloppy handwritten notes is intrinsically hard: the notes are scaffolds taken during Zoom meetings where most of the verbal explanation was never written down. Trying to reconstruct "what's on the page" from the page is the wrong problem.

The right problem: the notes exist to explain Rodina's published paper. Rodina's paper is the ground truth. Rewrite each notes page into clean exposition aimed at bridging the reader toward the relevant Rodina passages, with inline citation tags wherever the rewrite leans on a specific passage. The order the notes appear in doesn't matter if they're poorly organized — Claude can reorganize. The hand-drawn diagrams don't matter for this pass — Claude can describe what they were conveying in one line of prose and omit them. The "voice" of the rewrite should match Rodina's compact definition-first theorist style.

### What shipped this session

**1. `prompts/notes-rewrite.txt` — new system prompt.** Defines voice ("match Rodina's compact, definition-first style; no 'let us consider'"), LaTeX rigor rules (matched `\left`/`\right`, braced sub/superscripts, no bare `\hspace`), the `[[cite bookId="..." page="..."]]verbatim quote[[/cite]]` format with strict grounding rules (quote must be copied verbatim from the provided GROUND_TRUTH block, no fabrication), and a hard "better zero citations than fake ones" rule for pages that cover pure prereq material distant from the paper. Output format is the same `<div class="page-content">` + `<div class="math-display">` wrapping the existing reader expects.

**2. `scripts/rewrite-notes-page.js` — single-page one-shot rewriter.** Used for iteration. Given a notes bookId + page number, it: loads the notes Page doc and its chunks, concatenates them with the page rawText to form the query text, embeds that, scores every chunk from each linked source book by cosine similarity, takes the top K (default 12) sorted by source page number so they arrive in paper order, reads the page PNG as base64, and sends all of it plus the system prompt to Claude. Writes the result to `/tmp/rewrite-<bookId>-p<pageNumber>.html`. Does NOT touch the DB — this script is purely for iteration.

**3. `public/rewrite-preview.html` — standalone preview shell.** Dark background, MathJax 3 loaded from CDN, aggressive CSS overrides to prevent `reader.css` from leaking in and painting `.math-display` blocks as white boxes (the first preview attempt showed exactly that bug). The rewritten HTML fragment gets injected into the shell. First round of feedback from the user on page 1 was "looks way better" — the LaTeX renders, the reorganization is pedagogically sensible, and Claude correctly refused to fabricate citations when the top-12 Rodina chunks weren't relevant to Lagrangian mechanics.

**4. `scripts/rewrite-notes-pages-batch.js` — batch rewriter.** Takes a range (`from`..`to`), preloads all linked source-book chunks into memory once, loops serially through the pages. Writes results directly to `Page.htmlContent` and stashes the pre-rewrite HTML in a new `Page.htmlContentLegacy` field (added to the Page schema) the first time a page is rewritten, so re-running is idempotent. Prints a per-model token + per-lever cost breakdown at the end.

**5. Opus/Sonnet cost-routing.** `chooseModel()` inspects the notes-page text for Rodina-native vocabulary — 19 regex patterns: `hidden\s*zero`, `bcfw`, `tr\s*\(\s*[φϕ\\phi]\^?3?`, `kinematic\s*mesh`, `scattering\s*amplitude`, `feynman\s*diagram`, `yang[-\s]*mills`, `nlsm|nonlinear\s*sigma`, `colored\s*scalar`, `splittings?`, `factoriz`, `unitarity`, `locality`, `residue|pole`, `uv\s*scaling|ultraviolet`, `pion|gluon`, `amplitude\s*zero`, `gauge\s*invariance`, `soft\s*(theorem|limit)`. Any match → Opus 4.6 (the bridge pages that matter). Otherwise → Sonnet 4.6 (5× cheaper, indistinguishable for straight prereq exposition). On the Lagrangians book the first 28 pages routed 23 sonnet / 5 opus, matching expectation: classical mechanics in polar coords, Gaussian integrals, path integral setup are all sonnet; pages that touch Feynman diagrams or amplitude-adjacent content promote to opus.

**6. Prompt caching (partially working — see caveat).** System prompt marked `cache_control: { type: 'ephemeral' }`. Intent: pay a ~25% write premium on the first call, then every subsequent call within 5 minutes reads the cached system prompt at a 90% discount. **Caveat discovered mid-batch:** the notes-rewrite system prompt is ~600 tokens, which is below Anthropic's 1024-token cache minimum for Sonnet/Opus. The SDK silently ignores the cache_control hint in that case. Live output confirms: `cr=0 cw=0` on every call. The routing savings are real (~5× on all non-bridge pages) but the caching savings are not yet realized. Fix tracked as a new to-do item.

**7. The 33-missing-page bug + permanent auto-recovery.** Ran a DB audit early in the session: Lagrangians notes book showed `pageCount: 65` but only 32 Page documents in Mongo. Page numbers existing: `1,3,7,8,11,12,13,15,16,21,23,25,26,27,30,31,33,38,39,41,43,44,45,46,48,49,51,52,53,54,57,58`. Root cause was immediately visible in `ErrorLog`: 24+ entries, all `generate-html` jobs with `Page N: 429 Rate limit reached for gpt-4o in organization ... on tokens per min (TPM): Limit 30000`. The original parallel vision burst of 20-60 pages saturated the gpt-4o TPM budget, pages 27-65 hit 429s that were logged but never retried, the orchestrator moved on and marked the book `status='ready'` with silent holes. Every `page-N.png` exists on disk, so vision rendering was fine — it was the vision→HTML API call that failed.

Two fixes shipped for this:

- **One-shot repair: `scripts/fill-missing-pages.js`.** Takes a bookId, scans `uploads/images/<bookId>/` for `page-N.png`, compares against Page records, serially re-runs `convertPageWithVision()` on every missing pageNumber using the existing PNG (no re-render), saves the resulting HTML as a new Page document with `visionProcessed: true`. 800 ms throttle between calls to stay well under 30k TPM. Ran against the Lagrangians book: **33/33 pages repaired in 350 seconds, zero failures.** Book is now complete at 65/65.

- **Durable fix: `reconcilePages()` in `services/jobService.js`.** Called at the end of the `generate-html` job right before the book transitions from `processing` to `ready`. Scans the book's image directory, compares against Page records that are both present AND `visionProcessed: true`, collects the list of page numbers that are either entirely missing or flagged `visionProcessed: false`, and serially retries them using the existing PNG on disk. Up to 2 rounds, 800 ms delay between calls. Every failure gets logged to `ErrorLog` with `jobType: 'generate-html'` and `message` prefixed `reconcile page N:`. This is the permanent fix the user asked for: future uploads will self-heal from any TPM-induced page drops without manual intervention.

### Partial results of the full 65-page rewrite pass

In-flight background job `bqbzu9v9v` at the time of writing. Through page 28, the routing distribution was:
- Sonnet 4.6: 23 pages, avg 5.1k input / 1.5k output / 26s
- Opus 4.6: 5 pages, avg 5.3k input / 2.2k output / 40s

Cache tokens `cr=0 cw=0` on every call (see caveat #6 above). The final cost rollup and wall time will be logged in the next update entry once the batch completes.

### What's still to do after this session

- **Fix the prompt cache miss.** Three options: (a) pad the system prompt to >1024 tokens with more rigorous rules (the prompt could genuinely use stricter citation / LaTeX rules — probably the right move), (b) move the cache boundary into the user content (bundle the rules + a stable Rodina top-100-chunks block into one big cached user message), (c) accept the miss and rely on routing for savings. Option (a) is cheap and improves quality at the same time.
- **Run Phase 2 pipeline (chunking + spans + embeddings + edges) on the 33 newly-filled pages.** They have `htmlContent` and `rawText` now but no chunks, so the downstream `note-citation` edges from those pages don't exist yet.
- **Verify the rewrite actually emits `[[cite]]` tags on the Opus-routed bridge pages.** The opus pages touch amplitude-adjacent material and should surface real Rodina citations. Need to eyeball page 20, 24 in the preview once the batch lands.
- **Citation-click → wrong page bug.** Still open, but deferred: if the rewrite preserves `pageNumber` as the anchor, the existing jump logic should just work. Will verify after the batch lands.
